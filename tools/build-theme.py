...
{% else %}
  <img src="{{ fallback | asset_url }}" alt="{{ alt | escape }}" class="{{ class }}" loading="{{ loading | default: 'lazy' }}" width="{{ width | default: 800 }}" height="{{ height | default: 500 }}">
{% endif %}
''')
...
<footer class="site-footer wrap">
  ...
  <div class="social-links">{% if section.settings.instagram != blank %}<a href="{{ section.settings.instagram }}" aria-label="Instagram">{% render 'cellulaire-icon', name: 'instagram' %}</a>{% endif %}{% if section.settings.tiktok != blank %}<a href="{{ section.settings.tiktok }}" aria-label="TikTok">TikTok</a>{% endif %}{% if section.settings.facebook != blank %}<a href="{{ section.settings.facebook }}" aria-label="Facebook">f</a>{% endif %}{% if section.settings.youtube != blank %}<a href="{{ section.settings.youtube }}" aria-label="YouTube">YouTube</a>{% endif %}</div>
...
<section class="hero hero--{{ section.settings.variant }} {% if section.settings.composite_mobile and section.settings.mobile_image == blank %}hero--composite{% endif %} wrap">
  <div class="hero-copy">
  ...
  <div class="hero-image desktop-only">{% render 'cellulaire-image', image: section.settings.image, fallback: section.settings.fallback, alt: section.settings.image_alt, loading: 'eager', sizes: '60vw' %}</div>
  <div class="hero-image mobile-only">{% render 'cellulaire-image', image: section.settings.mobile_image, fallback: section.settings.mobile_fallback, alt: section.settings.image_alt, loading: 'eager', sizes: '100vw' %}</div>
</section>
''',[setting('select','variant','Layout','home',options=[dict(value=x,label=x.title()) for x in ['home','about','contact']]),setting('text','eyebrow','Eyebrow'),setting('textarea','heading','Heading','Where Science\nMeets Luxury'),setting('textarea','description','Description','Advanced skincare. Visible results. A refined everyday ritual.'),setting('text','button_label','Button','Shop face mists'),setting('url','button_link','Button link'),setting('image_picker','image','Desktop image'),setting('image_picker','mobile_image','Mobile image'),setting('checkbox','composite_mobile','Use supplied composite mobile banner',False,info='The PDF mobile banner contains lettering. Accessible text and a working link overlay are retained. Upload a clean photo to use native visible text.'),setting('text','fallback','Bundled desktop asset','cellulaire-hero.webp'),setting('text','mobile_fallback','Bundled mobile asset','cellulaire-hero-mobile.webp'),setting('text','image_alt','Image description','Cellulaire face mist ritual')])
...
<section class="skin-needs wrap inset">
  ...
  <div class="need-grid">{% for block in section.blocks %}{% assign destination = block.settings.collection.url | default: routes.all_products_collection_url %}<a class="need-card need--{{ block.settings.tone }}" href="{{ destination }}" {{ block.shopify_attributes }}>
...
''',[setting('text','heading','Heading','Shop by Skin Need')],[dict(type='need',name='Skin need',settings=[setting('text','heading','Title','Night Care'),setting('text','description','Description','Repair & renew'),setting('collection','collection','Collection'),setting('image_picker','image','Image'),setting('text','fallback','Bundled asset','cellulaire-need-night.webp'),setting('text','tone','Color tone','night')])])
...
<article class="product-card">
  ...
  <div class="product-card-copy"><h3><a href="{{ destination }}">{{ title | escape }}</a></h3><p>{{ product.metafields.custom.short_description | default: description | escape }}</p>
    {% if product != blank %}<span>{{ product.price | money }}</span>{% endif %}
    {% if product != blank and product.has_only_default_variant and product.available %}
...
<section class="products-section wrap inset {% if section.settings.compact %}products--compact{% endif %}">
  ...
  <div class="product-grid">
    {% if section.blocks.size > 0 %}{% for block in section.blocks %}<div {{ block.shopify_attributes }}>{% render 'cellulaire-product-card', product: block.settings.product, title: block.settings.title, description: block.settings.description, fallback: block.settings.fallback %}</div>{% endfor %}
    {% elsif section.settings.collection != blank %}{% for product in section.settings.collection.products limit: 4 %}{% render 'cellulaire-product-card', product: product, fallback: 'cellulaire-product-night.webp' %}{% endfor %}
    {% else %}{% for product in collections.all.products limit: 4 %}{% render 'cellulaire-product-card', product: product, fallback: 'cellulaire-product-night.webp' %}{% endfor %}{% endif %}
  </div>
</section>
''',[setting('text','eyebrow','Eyebrow'),setting('text','heading','Heading','Best Sellers'),setting('text','description','Description','Our most-loved face mists for healthy, radiant skin.'),setting('collection','collection','Collection'),setting('checkbox','compact','Compact cards',False)],[dict(type='product',name='Product',settings=[setting('product','product','Shopify product'),setting('text','title','Fallback title','Night Care Face Mist'),setting('text','description','Short description','Repair. Renew. Rebalance.'),setting('text','fallback','Bundled photo','cellulaire-product-night.webp')])])
...
<section class="routine wrap inset"><div><p>The Complete Routine</p><h2><span class="desktop-only">{{ section.settings.heading | escape }}</span><span class="mobile-only">The Complete Routine</span></h2><p>Elevate your ritual with our curated collection.</p></div><span class="save-badge">{{ section.settings.badge | escape }}</span><div class="routine-image">{% render 'cellulaire-image', image: section.settings.product.featured_image, fallback: 'cellulaire-routine.webp', alt: 'All four face mists', width: 147, height: 85 %}</div><div class="routine-product"><h3>{{ section.settings.product.title | default: 'The Signature Set' | escape }}</h3><p>All Four Face Mists</p>{% if section.settings.product != blank %}<p>{{ section.settings.product.price | money }} {% if section.settings.product.compare_at_price > section.settings.product.price %}<s>{{ section.settings.product.compare_at_price | money }}</s>{% endif %}</p>{% endif %}{% if section.settings.product != blank and section.settings.product.available and section.settings.product.has_only_default_variant %}{% form 'product', section.settings.product, data-product-form: '' %}<input type="hidden" name="id" value="{{ section.settings.product.selected_or_first_available_variant.id }}"><input type="hidden" name="quantity" value="1"><button class="button" type="submit">Add to bag</button><p data-form-status role="status"></p>{% endform %}{% else %}<a class="button" href="{{ section.settings.product.url | default: routes.all_products_collection_url }}">Shop the set →</a>{% endif %}</div></section>
...
<section class="contact-section wrap inset"><aside class="contact-details"><h2>Get in Touch</h2><p>Reach out through any of the channels below.<br>Our team will respond as soon as possible.</p>
  ...
  {% if section.settings.whatsapp != blank %}<a class="contact-channel" href="{{ section.settings.whatsapp }}">{% render 'cellulaire-icon', name: 'phone' %}<span><strong>WhatsApp</strong>Chat with us</span><span class="mobile-only">›</span></a>{% endif %}
...
products=[block('product',title=title+' Face Mist',description=description,fallback='cellulaire-product-'+name+'.webp') for name,title,description in [('night','Night Care','Repair. Renew. Rebalance.'),('hydration','Hydration','Refresh. Hydrate. Plump.'),('sun','Sun Protection','Defend. Soothe. Glow.'),('brightening','Brightening','Even. Brighten. Illuminate.')]]
needs=[block('need',heading=title,description=desc,fallback='cellulaire-need-'+name+'.webp',tone=name) for name,title,desc in [('night','Night Care','Repair & renew'),('hydration','Hydration','Refresh & plump'),('sun','Sun Protection','Defend & glow'),('brightening','Brightening','Even & radiate')]]
...
template('index',[
 item('hero','cellulaire-hero',dict(composite_mobile=True)),item('benefits','cellulaire-benefits',blocks=[block('benefit',icon=icon,text=text) for icon,text in [('drop','Science-backed\nformulas'),('leaf','Premium, clean\ningredients'),('diamond','Visible results,\neveryday'),('heart','A ritual\nof self-care')]]),
...
template('page.our-story',[
 ...
 item('values','cellulaire-values',blocks=values),item('formulas','cellulaire-editorial',dict(layout='formulas',eyebrow='',heading='Elevated Formulas.\nReal Results.',description='From our signature Face Mists to every skincare essential, each formula is designed to deliver effective performance while offering a luxurious, sensorial experience.',button_label='Discover our formulas',button_link='/pages/science',fallback='cellulaire-formulas.webp',aside='Same\nexceptional\nscience\na brighter you')),
